 ...
 * }
 */
router.get('/profile', authenticateSSO, async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
  try {
 ...
 * }
 */
router.get('/tenants', authenticateSSO, async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
  try {
    const userId = (req as any).ssoUser.userId;
    ...
    const tenants = userTenants.map((tenant) => {
      ...
      const apps = [
        ...
        },
        {
          appId: 'admin',
          name: 'Admin Panel',
          url: `https://admin.${process.env.BASE_DOMAIN || 'empire.com'}`,
